#!/usr/bin/env python3
"""
HITD Maps - Hunting States Scraper
Minnesota and Wyoming statewide parcels (verified working)
"""
...
# Verified working statewide APIs for hunting states
SOURCES = {
    "mn_statewide": {
        "name": "Minnesota - Statewide Parcels (55+ Counties)",
    ...
    "wy_statewide": {
        "name": "Wyoming - Statewide Parcels (23 Counties)",
        "url": "https://services3.arcgis.com/r0iJ85SKZ4zAzz3P/arcgis/rest/services/Wyoming_Parcels_for_2025/FeatureServer/0",
        "state": "WY",
        "records": 373048,
        "priority": 1,
    },
...
def download_arcgis_features(source_id, source, workers=40, batch_size=2000):
    ...
    def fetch_batch(offset):
        ...
        try:
            ...
            resp = requests.get(query_url, params=params, headers=HEADERS, timeout=120)
...
def main():
    parser = argparse.ArgumentParser(description='HITD Maps Hunting States Scraper')
    ...
    print("\n" + "="*80)
    print("HITD Maps - Hunting States Scraper (MN, WY)")
    print("="*80)
