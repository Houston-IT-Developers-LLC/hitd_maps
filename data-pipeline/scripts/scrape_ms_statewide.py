#!/usr/bin/env python3
"""
HITD Maps - Mississippi Statewide Parcel Scraper
Uses MS_East_Parcels and MS_West_Parcels for simplified download
Total: ~2M parcels (874K West + 1.12M East)
"""
...
# Mississippi uses two consolidated layers - East and West
SOURCES = {
    "ms_west": {
        "name": "Mississippi - West Parcels",
...
def download_arcgis_features(source_id, source, workers=40, batch_size=2000):
    """Download features from ArcGIS REST service using offset-based pagination"""
    url = source['url']
    name = source['name']
    ...
    # Calculate batches using offset pagination
    ...
    def fetch_batch(offset):
        """Fetch a single batch of features using offset"""
        try:
    ...
    # Download batches in parallel using offsets
...
def merge_and_upload_statewide():
    """Merge East and West into a single statewide file"""
    east_path = f"{OUTPUT_DIR}/pmtiles/parcels_ms_east.pmtiles"
    west_path = f"{OUTPUT_DIR}/pmtiles/parcels_ms_west.pmtiles"
    statewide_path = f"{OUTPUT_DIR}/pmtiles/parcels_ms_statewide.pmtiles"

    # Check if both parts exist locally
    if not os.path.exists(east_path) or not os.path.exists(west_path):
        print("  Both east and west PMTiles files needed for merge")
        return None

    print(f"\n  Merging East and West into statewide file...")

    # Use tile-join to merge
    cmd = [
        'tile-join',
        '-o', statewide_path,
        '--force',
    ...
    try:
        subprocess.run(cmd, check=True, capture_output=True)
        file_size = os.path.getsize(statewide_path) / (1024 * 1024)
        print(f"  Created: parcels_ms_statewide.pmtiles ({file_size:.1f} MB)")

        # Upload statewide
        cdn_url = upload_to_r2(statewide_path, "ms_statewide")
        if cdn_url:
            verify_upload("ms_statewide")
            # Cleanup all local files
            for path in [east_path, west_path, statewide_path]:
                if os.path.exists(path):
                    os.remove(path)
                    print(f"  Deleted local: {os.path.basename(path)}")
            return cdn_url
    except subprocess.CalledProcessError as e:
        print(f"  ERROR: tile-join failed: {e.stderr.decode()}")

    return None

def main():
    parser = argparse.ArgumentParser(description='HITD Maps Mississippi Statewide Scraper')
    parser.add_argument('--list', action='store_true', help='List available sources')
    parser.add_argument('--source', type=str, help='Specific source to process (ms_east, ms_west, or statewide)')
    ...
    if args.list:
        ...
        print(f"  statewide: Download both and merge (~2M records)")
        return

    print("\n" + "="*80)
    print("HITD Maps - Mississippi Statewide Parcel Scraper")
    print("="*80)

    if args.merge_only:
        merge_and_upload_statewide()
        return

    if args.source == 'statewide':
        # Download both parts then merge
        for source_id in ['ms_west', 'ms_east']:
            source = SOURCES[source_id]
            process_source(source_id, source, skip_existing=not args.no_skip, workers=args.workers)
        merge_and_upload_statewide()
        return
