#!/usr/bin/env python3
"""
Download parcel data for priority Georgia counties (Atlanta metro area).
Based on download_missing_states.py but customized for GA counties.
"""
...
# Output directory
OUTPUT_DIR = Path("/home/exx/Documents/C/hitd_maps/data-pipeline/downloads/ga_counties")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Georgia county endpoints
GA_COUNTIES = {
    'parcels_ga_clayton': {
        'url': 'https://weba.co.clayton.ga.us:5443/server/rest/services/TaxAssessor/ParcelSales/MapServer/0',
        'county': 'Clayton',
        'population': 297000,
        'max_per_request': 2000,
        'notes': 'Atlanta metro - parcel sales data (may timeout)'
    },
    'parcels_ga_columbia': {
        'url': 'https://gis.columbiacountymaps.com/server/rest/services/TaxlotWb/FeatureServer/0',
        'county': 'Columbia',
        'population': 156000,
        'max_per_request': 2000,
        'notes': 'Augusta area - taxlots'
    },
    'parcels_ga_bibb': {
        'url': 'https://services2.arcgis.com/zPFLSOZ5HzUzzTQb/arcgis/rest/services/TaxParcels/FeatureServer/0',
        'county': 'Bibb',
        'population': 153000,
        'max_per_request': 2000,
        'notes': 'Macon - tax parcels'
    },
    'parcels_ga_clarke': {
        'url': 'https://enigma.accgov.com/server/rest/services/ACC_Parcels/FeatureServer/0',
        'county': 'Clarke',
        'population': 128000,
        'max_per_request': 2000,
        'notes': 'Athens-Clarke County - parcels'
    },
}
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Download parcel data for Georgia counties')
    ...
    if args.list:
        print("Available Georgia counties:")
        for name, config in GA_COUNTIES.items():
            print(f"  {name}")
    ...
    counties_to_download = GA_COUNTIES
    if args.county:
        # Allow matching by county name or full key
        matched = None
        for key, config in GA_COUNTIES.items():
        ...
        if matched:
            ...
            print(f"Available: {[c['county'] for c in GA_COUNTIES.values()]}")
            return

    print("="*60)
    print("GEORGIA COUNTY PARCEL DOWNLOADER")
