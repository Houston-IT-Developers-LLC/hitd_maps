#!/usr/bin/env python3
"""
Scrape final missing states: North Carolina, South Dakota, and retry Rhode Island
"""
...
CONFIGS_TO_SCRAPE = [
    # North Carolina - try statewide first
    "NC_STATEWIDE",
    "NC_MECKLENBURG",  # Charlotte
    "NC_WAKE",  # Raleigh
    "NC_GUILFORD",  # Greensboro
    "NC_DURHAM",
    "NC_FORSYTH",  # Winston-Salem
    "NC_BUNCOMBE",  # Asheville
    # South Dakota
    "SD_MINNEHAHA",  # Sioux Falls
    "SD_PENNINGTON",  # Rapid City
    "SD_LINCOLN",
    "SD_CODINGTON",
    "SD_BEADLE",
    # Rhode Island retry
...
def check_projection_and_convert(geojson_path):
    """Check if coordinates need reprojection and convert to PMTiles."""
    import json

    # Sample first feature to check coordinates
    with open(geojson_path, 'r') as f:
        # Read just enough to get first coordinate
        content = f.read(10000)

    # Look for coordinate pattern
    if '"coordinates"' in content:
        # Find first coordinate
        idx = content.find('"coordinates"')
        coord_str = content[idx:idx+200]
        # Extract first number
        import re
        nums = re.findall(r'[-+]?\d*\.?\d+', coord_str)
        if nums:
            x = float(nums[0])
            # If X coordinate is > 180, it's probably projected
            if abs(x) > 180:
                return "projected"
    return "wgs84"


def convert_to_pmtiles(geojson_path, needs_reproject=False):
    pmtiles_path = str(geojson_path).replace('.geojson', '.pmtiles')

    if needs_reproject:
        # First reproject
        reprojected_path = str(geojson_path).replace('.geojson', '_wgs84.geojson')
        # Try common projections - State Plane NAD83
        for srs in ['EPSG:2264', 'EPSG:3358', 'EPSG:32119', 'EPSG:4326']:
            cmd = f'ogr2ogr -f GeoJSON -s_srs {srs} -t_srs EPSG:4326 {reprojected_path} {geojson_path} 2>&1'
            ok, _, err = run_cmd(cmd, timeout=1800)
            if ok and os.path.exists(reprojected_path) and os.path.getsize(reprojected_path) > 1000:
                geojson_path = reprojected_path
                break
    ...
    if ok and os.path.exists(pmtiles_path) and os.path.getsize(pmtiles_path) > 1000:
        return pmtiles_path
    return None
...
def process_config(config_id):
    ...
    proj_check = check_projection_and_convert(geojson_path)
    needs_reproject = proj_check == "projected"
    if needs_reproject:
        print(f"  [INFO] Coordinates appear projected, will reproject...")

    # Step 2: Convert to PMTiles
    print(f"  [2/3] Converting to PMTiles...")
    pmtiles_path = convert_to_pmtiles(geojson_path, needs_reproject)
...
def main():
    print("=" * 70)
    print("  SCRAPE FINAL MISSING STATES: NC, SD, RI")
    print("=" * 70)
